...
var obtenerSelectDatosEfiscal = function () {
    fetchDataArr(0, {}, 10, function (response) {
        if (response) {
...
var obtenerSelectDatosOS = function (txtEfiscal) {
    fetchDataArr(1, { _txtEfiscal: txtEfiscal }, 10, function (response) {
        if (response) {
            const select = $("#cboOs");
            select.empty();
            select.append($("<option>", { value: "0", text: "SELECCIONE" }));
...
var obtenerSelectDatosUP = function (txtEfiscal, txtOS, tipo) {
    fetchDataArr(2, { _txtEfiscal: txtEfiscal, _txtOS: txtOS }, 10, function (response) {
        if (response) {
            const select = $("#cboUp");
            select.empty();
            select.append($("<option>", { value: "0", text: "SELECCIONE" }));
...
function getReportesGA(eFiscal) {
    var html = '';
    blockUICustom();
    fetchDataArr(3, { _eFiscal: eFiscal }, 10, function (response) {
        if (response) {
                ...
                if (response.length !== 0) {
                    response.forEach(function (item) {
                        ...
                        html += `<tr>
                                        ...
                                        <td class='text-end'>
                                            <div>
                                                <button class='btn btn-sm btn-falcon-primary btnPreviewReporte customButton m-1' type='button'
...
function gDrp1See(idCtrlReporte, eFiscal) {
    ...
    fetchDataArr(8, { _idCtrlReporte: idCtrlReporte, _eFiscal: eFiscal }, 10, function (response) {
        if (response) {
            logger.log(`%cDatos devueltos sobre REPORTE TRIMESTRE ${response}`, "color: rgba(255, 50, 50, 0.2); font-weight: bold; background: #111; padding: 4px;");
...
async function setSign(data) {
    const setSign = await new Promise((resolve, reject) => {
        ...
        fetchDataArr(6, data, 10, (response) => {
            if (response) resolve(response);
            else reject(new Error("Error en fetchDataArr 6 Firma"));
        });
    });
    logger.log(`%cRESULTADO OBTENIDO DE AWAIT SYNC FIRMA ${setSign}`, "color: rgba(255, 50, 50, 0.2); font-weight: bold; background: #111; padding: 4px;");
    if (setSign) {
...
$(document).ready(function () {
    ...
    $(document).on("change", "#cboEfiscal, #cboOs, #cboUp", function () {
        var cveOS = $("#cboOs").val();
        var cveUP = $("#cboUp").val();
    ...
    $(document).on("click", ".btnR", async function () {
        ...
        fetchDataArr(5, { _idReporte: idReturn, _idUser: idUser, _eFiscal: eFiscal }, 8, function (response) {
            if (response) {
                logger.log(`%cRESULTADO SOBRE INSERT NUEVO CTRL REPORTE ${response}`, "color: rgba(255, 50, 50, 0.2); font-weight: bold; background: #111; padding: 4px;");
                var responseS = response.split("|");
                if (responseS[0] === "ok") {
    ...
    $(document).on("click", ".btnSignReporte", function () {
        ...
        fetchDataArr(4, { _idUsuario: idUser, __idRol: idRolUser }, 8, function (reponseSign) {
            if (reponseSign) {
                logger.log(`%cRESPUESTA DE DATOS USUARIO FIRMA ${reponseSign}`, "color: rgba(255, 50, 50, 0.2); font-weight: bold; background: #111; padding: 4px;");
    ...
    $(document).on("click", ".btnDownloadFirma", function () {
        ...
        fetchDataArr(7, { _idCtrl: idReturn1, _idNivel: idNivel }, 10, function (response) {
            if (response) {
    ...
    $(document).on("click", ".btnPreviewReporte", function () {
        const idReturn = $(this)[0].dataset.ctrlReporte;
        const idReturn2 = $(this)[0].dataset.reporte;
        var eFiscal = $("#cboEfiscal").val();
